...
Login uses a local phone/PC helper page instead of the Android TV WebView. Scan the QR code or open the URL shown on the TV from a device on the same network, log into Kick, paste a Kick Cookie header/session value, and submit. Refresh Login in Settings starts a new helper page when Kick expires the session.
